...
renderGroupItem selected cFocus sel (GUnstaged fp status _) =
...
renderGroupItem selected cFocus sel (GTracked fp _) =
  let a = if cFocus && sel then attrSelItem else attrItem
      mark = if Set.member fp selected then "\x25cf " else "  "
  in withAttr a $ str $ mark ++ fp
renderGroupItem selected cFocus sel (GUnassignedFile fp) =
  let a = if cFocus && sel then attrSelItem else attrItem
      mark = if Set.member fp selected then "\x25cf " else "  "
  in withAttr a $ str $ mark ++ fp
